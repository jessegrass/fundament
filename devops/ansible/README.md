...
DEPLOY_ENV=prod
DEPLOY_HOST=fundament.unordnung.org
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for fundament.unordnung.org, enable SSH, and install a supported version of Python 3 on that system.
...
cluster:
  hosts:
    fundament.unordnung.org:
      ansible_user: root
      ansible_host: fundament.unordnung.org
      host: fundament
      hostname: fundament.unordnung.org
      swarm_node:
        labels:
          type: manager
          env: production
...
Deploy the stack defined in `devops/stacks/fundament.unordnung.org.yml` to the remote server with:
